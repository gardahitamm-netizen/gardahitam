// ================================================================
//  GARDA HITAM — PATCH v4 (FINAL FIX)
//  Taruh SEBELUM </body> di HTML utama
//  HAPUS patch versi sebelumnya (v1/v2/v3)
// ================================================================
(function () {
  'use strict';
  ...
  //   - .running-bar & .breadcrumb pakai flex-shrink:0 ✓
  //   - Tapi .mixing-page-wrap dan .plant-wrap tidak
  //     punya explicit height → mereka mengembang ke ukuran konten
  //     bukan mengisi sisa space
  //   - Solusi: pastikan #pages-wrapper, .page.active, dan semua
  //     scroll wrapper pakai flex + min-height:0 yang benar
  document.head.insertAdjacentHTML('beforeend', `<style id="gh-v4-fix">
  ...
  /* ── Topbar, running-bar, breadcrumb: jangan shrink ── */
  .topbar        { flex-shrink: 0 !important; }
  .running-bar   { flex-shrink: 0 !important; }
  .breadcrumb    { flex-shrink: 0 !important; }
  ...
  /* ── Pages: inactive hidden, active fill ── */
  .page {
  ...
  /* Hero dan section TIDAK boleh flex:1, biarkan natural height */
  .plant-hero    { flex-shrink: 0 !important; }
  .plant-section { flex-shrink: 0 !important; }
  ...
  .mixing-hero    { flex-shrink: 0 !important; }
  .mixing-section { flex-shrink: 0 !important; }
  ...
  .content-tabs { flex-shrink: 0 !important; }
  ...
  /* Khusus tab yang punya inner scroll area sendiri */
  #tab-histori.active,
  #tab-manual.active,
  #tab-pool.active {
  ...
  /* Fix: switcher bar di summary/analisa jangan ikut scroll */
  .plant-switcher-bar { flex-shrink: 0 !important; }

  /* ── Mobile: lock viewport ── */
  ...
  // ── FIX: renderPlantPage dengan WS Boveri + Stat Clickable ──
  // Override lengkap agar konsisten dengan HTML asli
  window.renderPlantPage = async function () {
    ...
    try {
      ...
      const wbCard = `<div class="plant-card" onclick="navToPool('MCG','ws_boveri')"
        style="animation-delay:${wbIdx * 0.06}s;border-color:rgba(255,184,32,.45);background:linear-gradient(135deg,rgba(255,184,32,.07),rgba(255,184,32,.02))">
  ...
  // ── FIX: renderSummaryTab dengan breakdown INU/DSU/tipe ──
  window.renderSummaryTab = async function () {
    ...
    try {
      ...
      // ── Helper deteksi tipe ──
      const isINU = id => { const s = String(id || '').toUpperCase(); return s.includes('.INU.') || s.includes('-INU') || s.includes('_INU') || s.endsWith('INU'); };
      const isDSU = id => { const s = String(id || '').toUpperCase(); return s.includes('.DSU.') || s.includes('-DSU') || s.includes('_DSU') || s.endsWith('DSU'); };

      const TIPES = ['ACS880', 'ACS800', 'ACS580', 'ACS550', 'ACS355', 'ACS150', 'DCS880', 'DCS800'];
      const getTipe = d => {
        const sp = String((d.spesifikasi || '') + ' ' + (d.merk || '')).toUpperCase();
        return TIPES.find(t => sp.includes(t)) || (d.merk ? String(d.merk).toUpperCase().trim().slice(0, 10) : 'LAINNYA');
      };
      ...
      if (allK.length) {
        ...
        const tipeRowsHTML = Object.entries(cnt).sort((a, b) => b[1] - a[1]).map(([t, n]) => {
          ...
          return `<div style="display:flex;align-items:center;gap:10px;padding:6px 0;border-bottom:1px solid rgba(31,48,80,.4)">
            ...
            <div style="flex:1;background:var(--surf3);border-radius:3px;height:5px;overflow:hidden"><div style="width:${p}%;height:5px;background:${c.c}"></div></div>
        ...
        const unitRow = (lbl, n, clr, bg, bd) => n > 0 ? `<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
          ...
          <div style="flex:1;background:var(--surf3);border-radius:3px;height:5px;overflow:hidden"><div style="width:${Math.round(n / tot * 100)}%;height:5px;background:${clr}"></div></div>
          <span style="font-family:'JetBrains Mono',monospace;font-size:18px;font-weight:800;color:${clr}">${n}</span>
        </div>` : '';

        breakdownHTML = `<div style="background:var(--surf);border:1px solid rgba(126,184,232,.3);border-radius:14px;overflow:hidden;margin-bottom:20px;border-top:3px solid var(--kosong)">
      ...
      if (!adaKosong.length) {
        html += `<div class="state-box">
          ...
          <h3>Semua Drive Terisi</h3>
          <p>Tidak ada drive kosong di ${S.plant}</p>
        </div>`;
      } else {
        html += breakdownHTML;
  ...
  console.log('[GH-PATCH v4] loaded OK');
})();
